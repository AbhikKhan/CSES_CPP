...
void solve(){
    ll n;
    cin>>n;
    vt<pair<ll, ll>> guests(n);
    cin>>guests;
    map<ll, pair<vt<ll>, vt<ll>>> checkInOut;
    for(ll i = 0; i< n; ++i){
        checkInOut[guests[i].F].F.push_back(i);
        checkInOut[guests[i].S].S.push_back(i);
    }
    ll maxRooms = 0, r = 0;
    for(auto it = checkInOut.begin(); it != checkInOut.end(); it++){
        r += it->S.F.size();
        maxRooms = max(maxRooms, r);
        r -= it->S.S.size();
    }
    queue<ll> emptyRooms;
    for(ll i = 1; i<= maxRooms; ++i)emptyRooms.push(i);
    vt<ll> allocatedRooms(n, 0);
    for(auto it = checkInOut.begin(); it != checkInOut.end(); it++){
        for(auto ind: it->S.F)allocatedRooms[ind] = emptyRooms.front(), emptyRooms.pop();
        for(auto ind: it->S.S)emptyRooms.push(allocatedRooms[ind]);
    }
    cout<<maxRooms<<endl<<allocatedRooms<<endl;
}
int main(){
    speed_;
    ll t = 1;
