...
void solve(){
    ...
    // total longest decreasing subsequence == length of longest increasing subsequence and vice versa
    map<ll, ll> sub;
    loopP(i,0,n){
        auto it = sub.upper_bound(cubes[i]);
        if(it == sub.end())sub[cubes[i]]++;
        else{
            if(--sub[it->first] == 0)sub.erase(it->first);
            sub[cubes[i]]++;
        }
    }
    ll res = 0;
    for(auto it = sub.begin(); it != sub.end(); ++it)res += it->second;
    cout<<res<<endl;
}
int main(){
    speed_;
