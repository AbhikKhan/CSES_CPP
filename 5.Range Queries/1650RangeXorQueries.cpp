...
/**************************************************************************************/
vt<ll> tree;
ll getXor(int node, int low, int high, int query_low, int query_high){
    if(query_low <= low && query_high >= high)return tree[node];
    if(query_low> high || low > query_high || low> high)return 0;
    int last_in_left = (low + high)/ 2;
    return getXor(2*node, low, last_in_left, query_low, query_high) ^ 
                getXor(2*node+1, last_in_left+1, high, query_low, query_high);
}
...
void solve(){
    ...
    while(q--){
        cin>>a>>b;
        cout<<getXor(1, 0, n-1, a-1, b-1)<<endl;
    }
}
