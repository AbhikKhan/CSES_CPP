...
void solve(){
    ...
    repP(i,0,n){
        cin>>k;
        k += prefixSum.back();
        prefixSum.push_back(k);
    }

    set<pair<ll, ll>> values;
    for(ll i = a; i<= b; ++i){
        values.insert({prefixSum[i], i});
    }
    ll maxSum = values.rbegin()->first;
    // use prefixsum to get values in range b-a
    // slide the window and update the values and calculate again
    // do not end in n, decrement/ update the b-a window till the window become 0
    for(ll i = b+1; i<= n+b-a; ++i){
        values.erase({prefixSum[i-b+a-1], i-b+a-1});
        if(i<= n){
            values.insert({prefixSum[i], i});
        }
        maxSum = max(maxSum, values.rbegin()->first-prefixSum[i-b]);
    }
    cout<<maxSum<<endl;
}
