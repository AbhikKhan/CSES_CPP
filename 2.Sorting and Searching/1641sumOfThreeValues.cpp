...
void solve(){
    ...
    for(ll i = 0; i< n-2; ++i){
        ...
        ll l = i+1, r = n-1, target = k-nums[i][0];
        while(l< r){
            ll sum = nums[l][0] + nums[r][0];
            if(sum == target){
                vt<ll> pos = {nums[i][1], nums[l][1], nums[r][1]};
                sort(all(pos));
                cout<<pos<<endl;
                return;
            }
            else if(sum > target){
                while(l< r && nums[r][0] == nums[r-1][0])r--;
                r--;
            }
            else{
                while(l< r && nums[l][0] == nums[l+1][0])l++;
                l++;
            }
        }
    }
