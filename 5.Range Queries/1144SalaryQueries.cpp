...
/**************************************************************************************/
vt<vt<int>> tree;
int getSalary(int node, int query_low, int query_high, int h){
    if(query_high>= tree[node][1] && query_low<= tree[node][0]){
        return (1<<h);
    }
    if(query_high< tree[node][0] && query_low> tree[node][1]){
        return 0;
    }
    return getSalary(2*node, query_low, query_high, h-1) + getSalary(2*node+1, query_low, query_high, h-1);
}
...
void solve(){
    ...
    int n = 1<<h;
    tree.resize(2*n, vt<int>(2, 1e9+1));
}
