...
/**************************************************************************************/
vt<int> tree;
int getRoom(int node, int low, int high, int val){
    if(low == high){
        tree[node] -= val;
        return node;
    }
    ...
    int room = 0;
    if(tree[left]>= val){
        room = getRoom(left, low, (low + high)/2, val);
    }
    else if(tree[right] >= val){
        room = getRoom(right, (low + high)/2+1, high, val);
    }
...
void solve(){
    ...
    n = 1<<(int)ceil(log2(n));
    tree.resize(2*n);
    for(int i = 0; i< rooms.size(); ++i)tree[n+i] = rooms[i];
    for(int i = n-1; i>= 1; --i){
    ...
    while(q--){
        cin>>tourist;
        if(tourist > tree[1])cout<<0<<" ";
        else{
            int roomNo = getRoom(1, 0, n-1, tourist);
            cout<<roomNo-n+1<<" ";
        }
    }
    cout<<endl;
