...
/**************************************************************************************/
vt<int> tree;
int getIndex(int node, int low, int high, int rank){
    if(low == high){
        tree[node] = 0;
        return low;
    }
    ...
    int index = 0;
    if(tree[left]>= rank){
        index = getIndex(left, low, (low + high)/2, rank);
    }
    else {
        index = getIndex(right, (low + high)/2+1, high, rank - tree[left]);
    }
