...
/**************************************************************************************/
vt<ll> tree;
ll getSum(int node, int low, int high, int query_low, int query_high){
    if(query_low <= low && query_high >= high)return tree[node];
    if(query_low> high || low > query_high)return 0;
    int last_in_left = (low + high)/ 2;
    return getSum(2*node, low, last_in_left, query_low, query_high) + 
                getSum(2*node+1, last_in_left+1, high, query_low, query_high);
}

void update(int node, int low, int high, int query_low, int query_high, int val){
    if(query_low <= low && query_high >= high){
        tree[node] = val;
        return;
    }
    if(query_low> high || low > query_high)return;
    int last_in_left = (low + high)/ 2;
    update(2*node, low, last_in_left, query_low, query_high, val); 
    update(2*node+1, last_in_left+1, high, query_low, query_high, val);
...
void solve(){
    ...
    while(q--){
        cin>>s>>a>>b;
        if(s == 2)cout<<getSum(1, 0, n-1, a-1, b-1)<<endl;
        else update(1, 0, n-1, a-1, a-1, b);
    }
}
